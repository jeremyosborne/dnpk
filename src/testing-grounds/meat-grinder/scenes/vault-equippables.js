...
export const scene = async ({ protagonist: { armyGroup } }) => {
  ...
  const heroesHaveEquipment = _.some(heroes, (army) =>
    gameObjectsCommon.equipment.size(army)
  )
  const vault = dataSourceGame.vaultEquippables.get()

  out.t(
    "A vault-o-matic hums nearby, providing storage of and transdimensional, secure access to acquired items."
  )
  ...
  if (!heroes.length && !vault.length) {
    ...
    out.t(
      "(The video game logic in this world prevents normal troops from wielding artifacts.)"
    )
    return carryOn()
  ...
  const equipmentCandidatesFromChoices = () => {
    ...
    const candidates = _.filter(equippableCandidatesMap, (c) =>
      gameObjectsCommon.equipment.size(c)
    )
  ...
  while (stillEquipping) {
    ...
    _.forEach(equippableCandidatesMap, (container) => {
      ...
      if (gameObjectsCommon.equipment.size(equipment)) {
        out.t("{{container}}: {{equipment}}", {
          container: ui.text.naming.short(container),
          equipment: ui.text.naming.short(equipment),
        })
      } else {
        out.t("{{container}}: no equipment", {
          container: ui.text.naming.short(container),
        })
      }
    })
    ...
    if (action === "transfer") {
      ...
      const toTransfer = gameObjectsCommon.equipment.find(from, {
        id: equipmentId,
      })
