...
export const battleReport = function* ({
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    if (ev.name === "battle:round:start") {
      ...
      info.push(
        t(
          "{{attacker.fullName, colorize}} (str: {{attacker.strength}}) (health: {{attacker.health}}) vs. {{defender.fullName, colorize}} (str: {{defender.strength}}) (health: {{defender.health}})",
          { attacker, defender }
        )
      )
      ...
      info.push(
        t(
          "{{attacker.name, colorize}} (roll: {{attacker.roll}}) and {{defender.name, colorize}} (roll: {{defender.roll}})...",
          { attacker, defender }
        )
      )
      if (attacker.damaged || defender.damaged) {
        ...
        info.push(
          t("...{{woundedLabel, colorize}}: {{woundedName, colorize}}", {
      ...
      info.push(
        t("{{slainLabel, colorize}}: {{slainName, colorize}}", {
      ...
      throw new Error(
        `WARNING: fall through due to unrecognized event name: ${
          ev.name
        }; found on event: ${JSON.stringify(ev)}`
      )
    }
  }
  if (info.length) {
    throw new Error(
      `Battle report had unterminated battles. Info contains: ${JSON.stringify(
        info
      )}`
    )
  }
...
export const _createDeed = ({ terrain, turn }) => {
 ...
 */
export const createScene = ({ createAntagonist, createDeed = _createDeed }) =>
  _.flowRight([
    ...
    out.t("It's a fight.")
    out.t("")
    out.t("{{flag}} ({{armyGroup}})", {
      armyGroup: ui.text.naming.short(protagonistArmyGroup),
      flag: protagonistFlag,
    })
    out.t("vs.")
    out.t("{{flag}} ({{armyGroup}})", {
      armyGroup: ui.text.naming.short(antagonistArmyGroup),
      flag: antagonistFlag,
    })
    out.t("")
    await hitReturnToContinue("Hit return to charge into battle!")

    const { attackers, defenders, events } = battle({
      attackers: {
    ...
    if (defenders.survivors.length) {
      out.t("The opposing forces stand triumphant: ({{armyGroup}})", {
        armyGroup: ui.text.naming.short(defenders.survivors),
      })
    } else {
      out.t("The opposing forces have been destroyed.")
    }
    if (defenders.casualties.length) {
      out.t("Your armies destroyed the opposing troops: ({{armyGroup}})", {
        armyGroup: ui.text.naming.short(defenders.casualties),
      })
    } else {
    ...
    if (casualties.length) {
      out.t("A moment of silence for your fallen: ({{armyGroup}})", {
        armyGroup: ui.text.naming.short(casualties),
      })
    } else {
    ...
    if (equipment.length) {
      _.forEach(equipment, (equippable) =>
        dataSourceGame.vaultEquippables.add(equippable)
      )
      out.t("{{equipment}} shimmer away and teleport to the equipment vault.", {
        equipment: ui.text.naming.short(equipment),
      })
    }

    if (gameObjectsCommon.armies.size(protagonistArmyGroup)) {
      out.t("Your remaining troops march on: ({{armyGroup}})", {
        armyGroup: ui.text.naming.short(protagonistArmyGroup),
      })
    } else {
