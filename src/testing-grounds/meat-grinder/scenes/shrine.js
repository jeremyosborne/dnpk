 ...
 * @return {NextScene}
 */
export const scene = async ({ protagonist: { armyGroup }, terrain }) => {
  const deity = _.sample(gameObjectsCommon.dir("deity"))

  // Deity official names can have some funky characters.
  out.t(
    "You come upon the shrine of {{- deity}} surrounded by {{terrain}}. Your worthy armies will be blessed.",
    {
      deity: ui.text.naming.short(deity),
  ...
  _.forEach(armies, (army) => {
    if (gameObjectsCommon.effects.blessings.has(army, deity)) {
      out.t("{{army}} already has the blessing of {{- deity}}.", {
        army: ui.text.naming.short(army),
        deity: ui.text.naming.short(deity),
      })
    } else {
      gameObjectsCommon.effects.blessings.add(army, deity)
      out.t("{{army}} receives the blessing of {{- deity}}.", {
        army: ui.text.naming.short(army),
        deity,
      })
    }
