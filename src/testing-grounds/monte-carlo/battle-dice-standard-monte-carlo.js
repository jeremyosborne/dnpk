...
const validate = {
  isInteger: (value) =>
    _.isInteger(value) ? true : t("Value must be an integer."),
}
...
export const battleDiceStandardMonteCarlo = async () => {
  ...
  const counters = _.flow(
    ...
    (counts) =>
      _.map(counts, (count, roll) => ({ count, roll: _.parseInt(roll) })),
  ...
  _.forEach(counters, (counter) => {
    out(
      `${counter.roll}: ${counter.count} (${_.round(counter.count / times, 2)})`
    )
  })
  out("")
